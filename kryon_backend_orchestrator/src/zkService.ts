...
export class ZKOrchestrator {
    ...
    async submitFactoringProofToSoroban(
        ...
        advanceRequested: number,
        invoiceCommitment: Buffer,      // 32 bytes
        nullifier: Buffer,              // 32 bytes
        messageHash: Buffer,            // 32 bytes  SHA256 attestation from oracle
        oracleSignature: Buffer,        // 64 bytes  Ed25519 sig from oracle
        attestationTimestamp: number,   // Unix timestamp from oracle
    ) {
        ...
        const tx = new TransactionBuilder(
            await this.rpcServer.loadAccount(borrowerKeypair.publicKey()),
        ...
        .addOperation(
            contract.call('submit_zk_factoring',
                ...
                xdr.ScVal.scvAddress(
                    xdr.ScAddress.scAddressTypeAccount(borrowerKeypair.xdrPublicKey())
                ),
    ...
    async generateMerkleProof(
        leaf: string,
        pathElements: string[],  // 20 sibling hashes as hex strings
     ...
     */
    async submitKYCCredential(
        contractId: string,
        holderKeypair: Keypair,
        credentialType: number,
        credentialNullifier: Buffer,    // 32 bytes
        messageHash: Buffer,            // 32 bytes  oracle attestation SHA256
        oracleSignature: Buffer,        // 64 bytes  oracle Ed25519 signature
        attestationTimestamp: number,   // Unix timestamp from oracle
    ): Promise<any> {
        const contract = new Contract(contractId);
        ...
        const tx = new TransactionBuilder(
            await this.rpcServer.loadAccount(holderKeypair.publicKey()),
            { fee: '10000', networkPassphrase: Networks.TESTNET }
        )
        .addOperation(contract.call(
            ...
            // holder: Address
            xdr.ScVal.scvAddress(
                xdr.ScAddress.scAddressTypeAccount(holderKeypair.xdrPublicKey())
            ),
            // credential_type: u32
